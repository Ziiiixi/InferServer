...
void bgemm_internal_cublas<float>(CUDABLAS_BGEMM_ARGTYPES(float)) {
  ...
  std::ofstream log_file("/home/zixi/workspace/log/bgemm_internal_cublas.txt", std::ios_base::app);
  if (log_file.is_open()) {
    log_file << "bgemm_internal_cublas execution time: " << excution_time << " ms" << std::endl;
    log_file.close();
  } else {
...
void gemm_internal_cublas<float>(CUDABLAS_GEMM_ARGTYPES(float)) {
  ...
  float excution_time = 0;
  cudaEventElapsedTime(&excution_time, start, stop);
  std::ofstream log_file("/home/zixi/workspace/log/gemm_internal_cublas.txt", std::ios_base::app);
  if (log_file.is_open()) {
    log_file << "gemm_internal_cublas execution time: " << excution_time << " ms" << std::endl;
    log_file.close();
  } else {
    std::cerr << "Unable to open log file" << std::endl;
  }
...
void gemm_and_bias(
  ...
  float excution_time = 0;
  cudaEventElapsedTime(&excution_time, start, stop);
  std::ofstream log_file("/home/zixi/workspace/log/gemm_and_bias.txt", std::ios_base::app);
  if (log_file.is_open()) {
    log_file << "gemm_and_bias execution time: " << excution_time << " ms" << std::endl;
    log_file.close();
  } else {
    std::cerr << "Unable to open log file" << std::endl;
  }
