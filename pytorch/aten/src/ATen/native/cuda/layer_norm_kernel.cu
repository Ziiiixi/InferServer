...
void launch_vectorized_layer_norm_kernel(
  ...
  T_ACC* rstd_data
    ...
    std::ofstream log_file("/home/zixi/workspace/log/vectorized_layer_norm_kernel.txt", std::ios_base::app);
    if (log_file.is_open()) {
      log_file << "vectorized_layer_norm_kernel execution time: " << excution_time << " ms" << std::endl;
      log_file.close();
    } else {
