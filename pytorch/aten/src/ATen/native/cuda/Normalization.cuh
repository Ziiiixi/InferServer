...
void batch_norm_elemt_cuda_template(const Tensor& output_, const Tensor& input_, const Tensor& weight_,
  ...
  std::ofstream log_file("/home/zixi/workspace/log/batch_norm_transform_input_kernel.txt", std::ios_base::app);
  if (log_file.is_open()) {
    log_file << "batch_norm_transform_input_kernel execution time: " << excution_time << " ms" << std::endl;
    log_file.close();
  } else {
