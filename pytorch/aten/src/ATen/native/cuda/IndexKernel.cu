...
static void launch_kernel(const int64_t N, const func_t& f) {
  ...
  std::ofstream log_file("/home/zixi/workspace/log/index_elementwise_kernel.txt", std::ios_base::app);
  if (log_file.is_open()) {
    log_file << "index_elementwise_kernel execution time: " << excution_time << " ms" << std::endl;
    log_file.close();
  } else {
