...
// compute number of nonzero elements
  ...
  std::ofstream log_file("/home/zixi/workspace/log/DeviceReduceSingleTileKernel.txt", std::ios_base::app);
  if (log_file.is_open()) {
    log_file << "DeviceReduceSingleTileKernel execution time: " << excution_time << " ms" << std::endl;
    log_file.close();
  } else {
  ...
  if (self.dim() > 0) {
    ...
    float excution_time = 0;
    cudaEventElapsedTime(&excution_time, start, stop);
    std::ofstream log_file("/home/zixi/workspace/log/DeviceSelectSweepKernel.txt", std::ios_base::app);
    if (log_file.is_open()) {
      log_file << "DeviceSelectSweepKernel execution time: " << excution_time << " ms" << std::endl;
      log_file.close();
    } else {
      std::cerr << "Unable to open log file" << std::endl;
    }
    if (num_nonzeros_h > 0 && self.dim() > 1){
        ...
        float excution_time = 0;
        cudaEventElapsedTime(&excution_time, start, stop);
        std::ofstream log_file("/home/zixi/workspace/log/write_indices.txt", std::ios_base::app);
        if (log_file.is_open()) {
          log_file << "write_indices execution time: " << excution_time << " ms" << std::endl;
          log_file.close();
        } else {
          std::cerr << "Unable to open log file" << std::endl;
        }
