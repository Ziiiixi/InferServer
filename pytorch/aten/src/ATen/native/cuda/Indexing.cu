...
#define SMALL_INDEX(TENSOR_TYPE, INDICES_TYPE, TYPE, DST_DIM, SRC_DIM, IDX_DIM)         \
  ...
  std::ofstream log_file("/home/zixi/workspace/log/indexSelectSmallIndex.txt", std::ios_base::app);
  if (log_file.is_open()) {
    log_file << "indexSelectSmallIndex execution time: " << excution_time << " ms" << std::endl;
    log_file.close();
  } else {
...
#define LARGE_INDEX(TENSOR_TYPE, INDICES_TYPE, TYPE,                           \
  ...
  float excution_time_1 = 0;
  cudaEventElapsedTime(&excution_time_1, start_1, stop_1);
  std::ofstream log_file_1("/home/zixi/workspace/log/indexSelectLargeIndex.txt", std::ios_base::app);
  if (log_file_1.is_open()) {
    log_file_1 << "indexSelectLargeIndex execution time: " << excution_time_1 << " ms" << std::endl;
    log_file_1.close();
  } else {
    std::cerr << "Unable to open log file" << std::endl;
  }
