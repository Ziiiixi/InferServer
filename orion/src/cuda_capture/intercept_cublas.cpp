...
typedef cublasStatus_t (*cublasSgemm_v2_t)(cublasHandle_t, cublasOperation_t, cublasOperation_t, int, int, int, const float*, const float*, int, const float*, int, const float*, float*, int);
...
cublasStatus_t cublasSgemm_v2(cublasHandle_t handle, cublasOperation_t transa, cublasOperation_t transb, int m, int n, int k, const float *alpha, const float *A, int lda, const float *B, int ldb, const float *beta, float *C, int ldc) {
	// cublas_sgemm_func = (cublasSgemm_v2_t)dlsym(RTLD_NEXT, "cublasSgemm_v2");
	// if (!cublas_sgemm_func) {
	// 	fprintf(stderr, "Failed to resolve cublasSgemm_v2: %s\n", dlerror());
	// 	exit(EXIT_FAILURE);
	// }
	int idx = get_idx();
	assert (idx >= 0);
