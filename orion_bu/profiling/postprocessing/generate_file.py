...
# Remove commas from the 'Duration(ns)' column and convert to an integer
df['Duration(ns)'] = df['Duration(ns)'].str.replace(',', '').astype(int)
...
while i < num_rows:
    ...
    if 'cudnn' in x and 'LSTM' not in x:
        ...
        if ('bn_fw' in x) or ('bn_bw' in x):
            processed_kernel_names.append(['BatchNorm', row['Roofline_prof'], 0, row["SM_needed"], row["Duration(ns)"], row["Grid"], row["Block"]])
        elif (
            ('scudnn' in x)
            ...
            processed_kernel_names.append([x, row['Roofline_prof'], 0, row["SM_needed"], row["Duration(ns)"], row["Grid"], row["Block"]])
        ...
        processed_kernel_names.append(['Conv', row['Roofline_prof'], 0, row["SM_needed"], row["Duration(ns)"], row["Grid"], row["Block"]])
        ...
        tokens = x.split('<')
        found = 0
        processed_kernel_names.append([tokens[0], row['Roofline_prof'], 0, row["SM_needed"], row["Duration(ns)"], row["Grid"], row["Block"]])
    i += 1
