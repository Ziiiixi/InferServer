...
for index, row in df_raw.iterrows():
    ...
    # bytes = row[bytes_sec]
    cycles = row[cycles_sec] * 1e6 # convert from cycles/usecond to cycles/second
    bytes = row[bytes_sec] * 1e9
    ...
    if add or mul or fma:
        flops_sec = (add + mul + fma * 2) * cycles  # using correct cycles/sec
        ai = flops_sec / bytes  # AI in FLOPS per byte
        ai_list.append(ai)
        print(index, ai)
