...
df['Grid'] = df['Grid'].replace(',', '', regex=True).astype(int)
df['Number_of_threads'] = df['Number_of_threads'].replace(',', '', regex=True).astype(int)
df['Block'] = df['Block'].replace(',', '', regex=True).astype(int)
df['Static_shmem_per_block'] = df['Static_shmem_per_block'].replace(',', '', regex=True).astype(int)
df['Registers_Per_Thread'] = df['Registers_Per_Thread'].replace(',', '', regex=True).astype(int)
...
for index, row in df.iterrows():
    ...
    wraps_per_sm = floor((max_regs_sm / regs_per_wrap) / 4) * 4
