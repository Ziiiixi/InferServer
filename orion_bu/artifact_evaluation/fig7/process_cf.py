...
def load_kernel_info(model_name):
    """Load the kernel information CSV for a specific model."""
    return pd.read_csv(f'/home/zixi/orion_bu/artifact_evaluation/fig7/coexe_small_models//kernel_groups/{model_name}_groups.csv')

def load_contention_matrix(model_name):
    """Load the contention matrix for a specific model."""
    return pd.read_csv(f'/home/zixi/orion_bu/artifact_evaluation/fig7/coexe_small_models//contention_matrix/contention_matrix_{model_name}.csv')
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if match:
                ...
                # Only add the pair if both models are the same
                if model_1 == model_2:
                    profile_plan.append((kernel_1_id, kernel_2_id, model_1, model_2, cluster_1, cluster_2))
    
    return profile_plan

# Update the contention factor in the CSV based on the kernel pairs
...
log_file_path = '123.log'  # Adjust the path to your log file
...
for kernel_1_id, kernel_2_id, model_1, model_2, cluster_1, cluster_2 in profile_plan:
    ...
    kernel_1_info = load_kernel_info(model_1)
    kernel_2_info = load_kernel_info(model_2)
    
    # Get the group (cluster) of each kernel
    group_1 = get_kernel_group(kernel_1_id, kernel_1_info)
    group_2 = get_kernel_group(kernel_2_id, kernel_2_info)
    ...
    # Load the contention matrix for the model
    contention_matrix = load_contention_matrix(model_1)
    
    # Get the contention factor between the two groups
